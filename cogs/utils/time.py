...
class ShortTime:
    compiled = re.compile("""(?:(?P<years>[0-9])(?:years?|y|yr))?                   # e.g. 2y
                             (?:(?P<months>[0-9]{1,2})(?:months?|mo))?              # e.g. 2months
                             (?:(?P<weeks>[0-9]{1,4})(?:weeks?|w))?                 # e.g. 10w
                             (?:(?P<days>[0-9]{1,5})(?:days?|d))?                   # e.g. 14d
                             (?:(?P<hours>[0-9]{1,5})(?:hours?|h))?                 # e.g. 12h
                             (?:(?P<minutes>[0-9]{1,5})(?:minutes?|m|min|mins))?    # e.g. 10m
                             (?:(?P<seconds>[0-9]{1,5})(?:seconds?|s|sec|secs))?    # e.g. 15s
                          """, re.VERBOSE)
    ...
    async def convert(self, ctx, argument):
        ...
        try:
            ...
            if begin not in (0, 1) and end != len(argument):
                raise commands.BadArgument('Time is either in an inappropriate location' 
...
def human_timedelta(dt, *, source=None, accuracy=3, brief=False, suffix=True):
    ...
    if len(output) == 0:
        ...
        if not brief:
            ...
            return ' '.join(output) + suffix
>>>>>>> d3afe47bb694fb716b41425b74c014a4e42a05b9
